...
<html lang="en">
  <head>
  	<title>SALES Record - <?php echo $branch_name;?></title>
    <meta charset="utf-8">
...
<link rel="icon" href='images/fav-icon.ico'>
  ...
  <body>
	...
	<h3 style='text-align:center;'>INSERT SALES RECORDS</h3>
		<div class='sales'><a href='sales_type.php'><button class='btn btn-primary' >+SALES TYPE</button></a></div>
		<div style='text-align:center;'><form method="post" enctype="multipart/form-data" action="upload.php">
						<td><b>UPLOAD SALES RECORDS</b></td><td><input type="file" name="salesfiles[]" class="btn btn-default" id="fileToUpload" required title="file should only be in CSV format only" multiple="multiple"></td>
  ...
  <form id='week_data'>
		...
		<?php
			...
			foreach($property_decode['sales_type'] as $cat){
							...
							echo "<td><input type='checkbox' name='".$cat."_id' class='form-control' onClick='enable(this.name)'></td>
			<td>$cat</td>

...
<h3 style='text-align:center;'><b>Monthly Report <? echo $branch_name?></b></h3>
		 ...
		 <form class="form-inline justify-content-center" style="padding:10px" method="POST" action="yearly_report.php" >
           ...
           $curr_year = isset($_POST['year'])? $_POST['year'] : date("Y");
			for($i=2018;$i<=3000;$i++){
...
$expenses_collection = [];
$profit_collection = []; 
$expenses_array_collection = [];
for($mothloop=0;$monthloop<12;$monthloop++){
echo $month_start = date("Y-m-d",  $time);
$month_end = date("Y-m-t",$time);
//THIS IS WHERE WE COLLECT ALL THE DATA

//SALES COLLECTION
echo $query = "select sum(c36) as sales from t107 where c13='$branch' and c23 between '$month_start' and '$month_end' ";
$result=mysqli_query($con,$query);
$row = mysqli_fetch_assoc($result); 
$sum = round($row['sales'],2);
$sales_collection[date("m",$time)] = $sum;
//EXPENSES COLLECTION
 $query = "SELECT c20,sum(c30) FROM t104 left join t106 on t104.c19=t106.c19 where t104.c23 between '$month_start' and '$month_end' group by c20";
$result=mysqli_query($con,$query);
if (mysqli_num_rows($result) > 0) {
    while($row = $result -> fetch_row()) {
        $expenses_array_collection[date("m",$time)][$row[0]] = $row[1];
    }

$expenses_collection[date("m",$time)] = array_sum($expenses_array_collection[date("m",$time)]);
}


//PAYROLL RECORD COLLECTION
date_default_timezone_set("America/Chicago");
	$total_pay=0;
    $in=strtotime($month_start.' 00:00:01') * 1000;
    $out=strtotime($month_end.' 23:59:59') * 1000;
...
$total_pay+=$cal;
	...
	// $total_exp+=$total_pay;
    if(array_key_exists(12,$expenses_collection))
        $expenses_collection[date("m",$time)] +=$total_pay;
    else
    $expenses_collection[date("m",$time)]=$total_pay;
    $expenses_array_collection[date("m",$time)]['payroll']=$total_pay;
//PROFIT CALCULATION
...
echo "sales: ";
print_r($sales_collection);
echo "Expenses: ";
print_r($expenses_collection);

print_r($expenses_array_collection);

...
<html lang="en">
  <head>
  	<title>ADD CATEGORY - <?php echo $branch_name;?></title>
    <meta charset="utf-8">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
              <ul class="nav navbar-nav ml-auto">
                 <li class="nav-item active ">
                    <b><a class="nav-link" href="insert_data.php">Insert Data</a></b>
                </li>
	...
	if (mysqli_num_rows($result) > 0) {
				...
				foreach($row as $key=>$value)
				{
					if($key=='c19')
					{
						...
						{
							echo "<td><input type='text' value='$value' class='form-control' name='$link'></td>";
						}
				}
				echo "</tr>";
				}
		...
		if(!empty($_POST))
		{
			...
			foreach($_POST as $key=>$value)
			{
				...
				if($value=='')
						continue;
					$query="insert into t106 (c20) values('$value');";
				...
				{
					if($value=='')
						$query="delete from t106 where c19='$key';";
					else
						$query="update t106 set c20='$value' where c19='$key';";
					if(mysqli_query($con,$query))
					{
					$flag=2;
					}
		...
		if($flag==1 || $flag==2)
		{
			echo "<div class='alert alert-success' id='alert_box' role='alert'> Chategory Data Updated Successfully!!</div>";
		echo "  <meta http-equiv='refresh' content='1'>";
		//die();
			
		}
		else
			echo "<div class='alert alert-danger' id='alert_box' role='alert'> Failed to update category data!</div>";
